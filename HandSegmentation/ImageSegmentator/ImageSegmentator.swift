...
class ImageSegmentator {
  ...
  func runSegmentation(
    onFrame pixelBuffer: CVPixelBuffer, completion: @escaping ((Result<SegmentationResult>) -> Void)
  ) {
    tfLiteQueue.async {
      ...
      do {
        self.image = UIImage.init(pixelBuffer: pixelBuffer)!
        
       guard
        let rgbData = self.image.scaledData(
      ...
      guard
        ...
        let overlayImage = self.image.overlayWithImage(image: resultImage, alpha: 0.5)
      else {
  ...
  private func coordinateToIndex(x: Int, y: Int, z: Int) -> Int {
    ...
    private func rgbDataFromBuffer(
      ...
      return Data(copyingBufferOf: rgbBytes.map { CFloat($0) / 255.0 })
    }
...
private enum Constants {
  ...
  static let labelsFileName = "hands_labels"
  static let labelsFileExtension = "json"

  /// The TF Lite segmentation model file
  static let modelFileName = "hands_finetunning_mobilenetv2_2019-11-20 01_54_18.774101"
